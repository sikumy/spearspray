...
def parse_arguments():
    ...
    ldap_group.add_argument('-dc', '--domain-controller', type=str, metavar="DOMAIN_CONTROLLER", help='FQDN (recommended) or IP address of the domain controller used for LDAP enumeration (required).')
    ldap_group.add_argument('-kdc', '--kdc', type=str, metavar="KDC", default=None, help='FQDN (recommended) or IP address of the KDC used for Kerberos spraying. Defaults to the value of -dc/--domain-controller if not set.')
